                                                  ...
                                                  uint64_t systick_ms)
{
   ...
   if(update_needed)
   {
      ...
      result = ifc->parent->_dock_data_manager_ifc->enqueue(
         ifc->parent->_dock_data_manager_ifc, DATA_ID_RING_STATUS, &status_to_enqueue, sizeof(ring_status_t), 1);
   }
...
static result_t dispatch_by_ppi(const ring_manager_interface_t *ifc, mp_packet_payload_t *rx_packet)
{
   ...
   if(IS_OK(result))
   {
      switch(rx_packet->ppi)
      {
         ...
         case PPI_RD_DOCKING_DATA:

            element_size = data_store[RING_MANAGER_DATA_ID_RING_DOCKED_STATUS].element_size;
            element_count = payload_len / data_store[RING_MANAGER_DATA_ID_RING_DOCKED_STATUS].element_size;
            data_id = data_store[RING_MANAGER_DATA_ID_RING_DOCKED_STATUS].id;

            // Check that the payload length is valid - There should be no remainder bytes when dividing by element size
            UPDATE_ERR_IF_TRUE(result, (0 != (payload_len % element_size)), RING_MANAGER_ERROR_INVALID_PAYLOAD_LENGTH);

            // Enqueue the received docking data
            IF_OK_RUN_AND_UPDATE(
               result,
               data_manager_ifc->enqueue(data_manager_ifc, data_id, rx_packet->payload, element_size, element_count));
            break;
                           ...
                           const rtc_system_time_interface_t *rtc_ifc)
{
   ...
   self->_pending_commands[CMD_ID_REQ_RING_DOCKED_STATUS_DATA].ppi = PPI_RD_DOCKING_DATA;
   self->_pending_commands[CMD_ID_REQ_RING_DOCKED_STATUS_DATA].type = PPI_TYPE_RQ;
   self->_pending_commands[CMD_ID_REQ_RING_DOCKED_STATUS_DATA].data_length = sizeof(uint16_t);
