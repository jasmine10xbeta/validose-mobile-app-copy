...
import { doesMatchReplacementStep4CheckingToSuccessSignal } from "@/constants/replacementFlow";
import {
...
export default function ReplaceMedicationStep4CheckingScreen() {
  ...
  useEffect(() => {
    ...
    async function watchStep4Signal() {
      try {
        unsub = await subscribeToReplacementFlowSignal(async (hex) => {
          ...
          if (doesMatchReplacementStep4CheckingToSuccessSignal(hex)) {
            ...
            hasNavigatedRef.current = true;
            router.replace(
              `/home/dashboard/replace-medication-success?startedAt=${startAtMs}`
            );
          }
        });
      } catch (error) {
        console.warn("[Replacement] Could not subscribe to step 4 error signal", error);
      }
    }
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        <View style={styles.topRow}>
          ...
          <View style={[styles.timerPill, isLastMinute && styles.timerPillCritical]}>
            ...
            <Feather
              name="rotate-ccw"
              size={20}
              color={isLastMinute ? "#C91F1F" : "#2A6574"}
            />
          </View>
        </View>
        ...
        <View style={styles.progressHeader}>
          ...
          <Text style={styles.progressRight}>95%</Text>
        </View>
        ...
        <View style={styles.imagePanel}>
          <Image source={CHECK_DOCK_TWO_IMAGE} style={styles.stepImage} resizeMode="cover" />
        </View>
