...
import {
  ...
  HEADER_LEN,
  MAX_TIME_BEFORE_SYNC_RETRY_MS,
  MESSAGE_PROTOCOL_MAX_PAYLOAD_LEN,
  MESSAGE_PROTOCOL_MIN_PAYLOAD_STRUCT_SIZE,
  MIN_PACKET_LEN,
} from "./messageProtocol.constants";
import {
  AckedRxDataPacket,
  BleMessageProtocolOptions,
  Logger,
  ...
  RxDataAckedHandler,
  RxHandler,
} from "./messageProtocol.interface";
import { createBleCharacteristicTransport } from "./messageProtocol.linkLayer";
import { buffersEqual, crc16Update, generateSessionId } from "./messageProtocol.utils";
...
export class BleMessageProtocol implements MessageProtocolInterface {
  ...
  private readonly onRxDataAcked?: RxDataAckedHandler;
  ...
  constructor(options: BleMessageProtocolOptions) {
    ...
    this.onRxDataAcked = options.onRxDataAcked;
    this.autoConsumeRx = options.autoConsumeRx ?? false;
    this.transport =
      options.transport ??
      createBleCharacteristicTransport({
        txCharacteristicUUID: options.txCharacteristicUUID,
        rxCharacteristicUUID: options.rxCharacteristicUUID,
  ...
  /**
   * Explicitly trigger a master-led sync cycle (SYNC_START -> SYNC_ACK).
   */
  async startSync(): Promise<MsgProtError> {
    if (!this.isMaster || !this.enableSyncControl) {
      return MsgProtError.INVALID_ARG;
    }
  ...
  send(payload: MpPacketPayload): MsgProtError {
    ...
    this.logger.info("[MP] DATA queued for TX.", {
      pktCounter: this.txPacket.header.pktCounter,
      sessionId: this.txPacket.header.sessionId >>> 0,
      ppi: this.txPacket.payload.ppi,
      type: this.txPacket.payload.type,
      payloadLen: this.txPacket.payload.pktPayloadLen,
    });
  ...
  private handleIncomingRaw(raw: Uint8Array): void {
    ...
    void this.onLinkLayerPacket(parsed.packet).catch((error) => {
      this.logger.error("[MP] Failed to process inbound packet.", error);
    });
  ...
  private async handleAckPacket(packet: MpPacket): Promise<void> {
    ...
    if ((waitingForAck && counterMatches) || canRelaxedComplete) {
      ...
      this.txPacket.header.status = MsgProtTxPacketStatus.COMPLETED;
      this.logger.info("[MP] ACK accepted; TX completed.", {
        pendingCounter: this.pendingId,
        receivedCounter: packet.header.pktCounter,
  ...
  private async handleNakPacket(packet: MpPacket): Promise<void> {
    ...
    if (result === MsgProtError.NONE) {
      this.logger.warn("[MP] NAK received; retransmitting last packet.", {
        pendingCounter: this.pendingId,
        receivedCounter: packet.header.pktCounter,
        sessionId: packet.header.sessionId >>> 0,
      });
  ...
  private async handleDataPacket(packet: MpPacket): Promise<void> {
    ...
    if (delivered === MsgProtError.NONE && !rxBusy && this.sendAckNak) {
      const ackResult = await this.sendAckOrNak(packet, true);
      if (ackResult === MsgProtError.NONE) {
        this.notifyRxDataAcked(packet);
      } else {
        this.logger.warn("[MP] Failed to ACK inbound DATA packet.", {
  ...
  private deliverPayload(packet: MpPacket): MsgProtError {
    if (this.rxPacket.header.status === MsgProtRxPacketStatus.PROCESSED) {
      ...
      try {
        ...
        this.logger.error("[MP] RX callback failed; preserving protocol flow.", error);
      }
  ...
  private async sendAckOrNak(packet: MpPacket, isAck: boolean): Promise<MsgProtError> {
    ...
    return this.sendPktToLinkLayer(ackPacket);
  }

  private notifyRxDataAcked(packet: MpPacket): void {
    if (!this.onRxDataAcked) {
      return;
    }

    const ackedPacket: AckedRxDataPacket = {
      pktCounter: packet.header.pktCounter,
      sessionId: packet.header.sessionId >>> 0,
      payload: {
        type: packet.payload.type,
    ...
    try {
      this.onRxDataAcked(ackedPacket);
    } catch (error) {
      this.logger.error("[MP] onRxDataAcked callback failed.", error);
    }
  }
  ...
  private getPacketTypeLabel(pktType: number): string {
    ...
    if (pktType === this.packetTypes.SYNC_START) return "SYNC_START";
    if (pktType === this.packetTypes.SYNC_ACK) return "SYNC_ACK";
    if (pktType === this.packetTypes.SYNC_MISMATCH) return "SYNC_MISMATCH";
    return "UNKNOWN";
  }
