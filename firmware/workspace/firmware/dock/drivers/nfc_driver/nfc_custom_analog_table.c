...
/*
 * Dock integration notes:
 *   - Tag V (ISO15693) behavior is governed by entries whose mode combines
 *     RFAL_ANALOG_CONFIG_TECH_NFCV and POLL/LISTEN. Update the ANT_TUNE_A/B
 *     register values in those MODE_ENTRY_* blocks to lock in production V-tag
 *     tuning.
 *   - Tag A (ISO14443A) uses the RFAL_ANALOG_CONFIG_TECH_NFCA entries. Adjust
 *     their antenna and TX driver values when NFC-A measurements change.
 * Each section below is labeled just like the ST default table, so you can
 * search for “NFCV” or “NFCA” to jump to the tag family you need to retune.
 */
...
/*! Macro for Configuration Setting with register-mask-value sets:
 *  - Configuration ID[2], Number of Register sets to follow[1], Register[2], Mask[1], Value[1], Register[2], Mask[1], Value[1], Register[2]... */
...
const uint8_t rfalAnalogConfigCustomSettings[] = {
    ...
    MODE_ENTRY_20_REG( (RFAL_ANALOG_CONFIG_TECH_CHIP | RFAL_ANALOG_CONFIG_CHIP_INIT)
                      ...
                      , ST25R3916_REG_ANT_TUNE_A, 0xFF, 0x80                                                                                                 /* Dock tuned ANT_TUNE_A */
                      , ST25R3916_REG_ANT_TUNE_B, 0xFF, 0x80                                                                                                 /* Dock tuned ANT_TUNE_B */
    ...
    /* Tag A (ISO14443A) TX power: edit TX_DRIVER and ANT_TUNE values here for NFC-A tag performance. */
    ...
    /* Tag A RX sensitivity: adjust RX_CONF/CORR_CONF registers per measured margins. */
    ...
    /* Tag V (ISO15693) TX tuning: adjust ANT_TUNE and AWS fields for the dock antenna. */
    ...
    /* Tag V RX front-end shaping: tweak thresholds if ISO15693 reception margins shift. */
...
const uint16_t rfalAnalogConfigCustomSettingsLength =
   (uint16_t)sizeof(rfalAnalogConfigCustomSettings);
