...
static const uint8_t THIS_UNIT_ID = (uint8_t)SW_UNIT_ID_BLE_CS_RING;
...
ret_code_t ble_cs_init(ble_cs_t *p_cs, const ble_cs_init_t *p_cs_init)
{
   ...
   RETURN_RET_CODE_IF_NRF_ERR(err_code);

   // Add Custom Value characteristics
   ble_uuid.uuid = UUID_DEVICE_COMAMNDS;
   uint8_t properties = BLE_GATT_CHAR_PROPERTIES_WRITE;
   err_code = add_characteristic(p_cs,
                                 p_cs_init,
                                 &ble_uuid,
                                 &p_cs->device_command_handle,
                                 properties,
                                 sizeof(uint32_t),
                                 sizeof(uint32_t),
                                 NULL,
...
#pragma GCC diagnostic pop
   ...
   if(IS_OK(result) && send_notify && (p_cs->conn_handle != BLE_CONN_HANDLE_INVALID))
   {
      ...
      err_code = sd_ble_gatts_hvx(p_cs->conn_handle, &hvx);
      if(err_code != NRF_SUCCESS)
      {
         DEBUG_ERROR("Failed to send notification: %d\n", err_code);
      }
      UPDATE_IF_NRF_ERR(err_code, result, BLE_CS_ERROR_NRF_ERROR);
   }
...
static void on_write(ble_cs_t *p_cs, ble_evt_t const *p_ble_evt)
{
   ...
   if(p_evt_write->handle == p_cs->device_command_handle.value_handle)
   {
      ble_cs_evt_t evt;
      evt.evt_type = BLE_CS_EVT_DEVICE_COMMANDS_UPDATED;

      // Set to value written to the characteristic. Little-endian uint32_t decode
      evt.device_command = (uint32_t)p_evt_write->data[3] << 24 | (uint32_t)p_evt_write->data[2] << 16
                           | (uint32_t)p_evt_write->data[1] << 8 | (uint32_t)p_evt_write->data[0];
