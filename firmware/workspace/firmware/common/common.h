...
#define RING_STATUS_T_SIZE_BYTES (42u)
typedef struct __attribute__((packed))
{
   ...
   uint8_t cap_detection_status;                /**< Cap detection status of the ring. See CAP_STATE enum */
} ring_status_t;
...
{
   uint16_t threshold;  /**< Proximity threshold for determining cap state. */
...
#define CAP_DETECTION_STATUS_T_SIZE_BYTES (7u)
...
/**
 * @brief Enumeration representing the cap state based on proximity sensor readings. This is used for reporting the cap
 * state in the ring status and other relevant data structures, as well as for processing cap state changes in the ring
 * firmware.
 */
typedef enum
{
